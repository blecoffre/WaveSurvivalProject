...
namespace NeoFPS
{
    ...
    public class NeoFpsAudioManager : NeoFpsManager<NeoFpsAudioManager>
    {
        ...
        [SerializeField, Tooltip("The number of pooled audio sources for 2D sound effects.")]
        private int m_Num2dEffectSources = 5;

        [SerializeField, NeoPrefabField(required = true), Tooltip("An optional prefab for the 2D effects sources. If not provided then the objects will be created from scratch.")]
        private AudioSource m_2dSourcePrefab = null;
        ...
        private static int s_Current2dEffectSourceIndex = -1;
        ...
        protected void OnValidate()
        {
            ...
            if (m_Num2dEffectSources < 1)
                m_Num2dEffectSources = 1;
        ...
        class RuntimeBehaviour : MonoBehaviour
        {
            ...
            protected void Awake()
            {
                ...
                if (instance.m_NumSpatialEffectSources > 0)
                {
                    ...
                    spatialEffectsSources = new List<AudioSource>(count);
                ...
                // Create 2D effects source
                if (instance.m_Num2dEffectSources > 0)
                {
                    // Allocate list
                    int count = instance.m_Num2dEffectSources;
                    stereoEffectsSources = new List<AudioSource>(count);

                    AudioSource prototype = instance.m_2dSourcePrefab;
                    if (prototype == null)
                    {
                        // Create the prototype
                        var sourceGO = new GameObject("2DEffectsSource (1)");
                        sourceGO.transform.SetParent(root);
                        prototype = sourceGO.AddComponent<AudioSource>();
                        prototype.outputAudioMixerGroup = spatialEffectsGroup;
                        prototype.playOnAwake = false;
                        prototype.bypassListenerEffects = false;
                        prototype.bypassReverbZones = true; // ???
                        prototype.bypassEffects = false;
                        prototype.loop = false;
                        prototype.spatialBlend = 0f;
                        stereoEffectsSources.Add(prototype);
                        --count;
                    }
                    ...
                    for (int i = 0; i < count; ++i)
                    {
                        var duplicate = Instantiate(prototype);
                        duplicate.transform.SetParent(root);
                        stereoEffectsSources.Add(duplicate);
                    }
                }
                ...
                if (instance.m_NumAmbienceOneShotSources > 0)
                {
                    ...
                    ambienceSources = new List<AudioSource>(count);
            ...
            public List<AudioSource> stereoEffectsSources
            {
                get;
                private set;
            }
        ...
        public static void Play2DEffectAudio(AudioClip clip, float volume = 1f)
        {
            // Check sources exist
            var sourceList = instance.m_ProxyBehaviour.stereoEffectsSources;
            if (sourceList == null || sourceList.Count == 0)
                return;

            // Get the index (looping)
            if (++s_Current2dEffectSourceIndex == sourceList.Count)
                s_Current2dEffectSourceIndex = 0;

            // Position and play
            var source = sourceList[s_Current2dEffectSourceIndex];
            source.PlayOneShot(clip, volume);
        }
